...
function covertToCastString(casts){
  ...
  for(var dic in casts)
    castsjson = castsjson + casts[dic].name + ' / ';
  var castFinal = castsjson.substring(0, castsjson.length-3);
  console.log(castFinal);
  return castFinal;
}

//处理演员信息：头像加名字
function getActorInfo(casts){
  var castArray = [];
